...
class FaceControlledMouse:
    ...
    def mouse_move(self, event, dx, dy):
            # -x = right, -y = down 
            multiplier = config.mouse_sens_var.get()
            ctypes.windll.user32.mouse_event(event, int(dx * multiplier), int(dy * multiplier), 0, 0)
    ...
    def wink_click(self, lwink_ratio, rwink_ratio):
        ...
        if not self.hold_left:
            ...
            # print('holding')
            if lwink_ratio > 0.24: # <-- Stops holding
                self.left_click = 0
                self.mouse_move(0x0004, 0, 0)
                self.hold_left = False
        ...
        while cap.isOpened():
            ...
            if results.multi_face_landmarks:
                ...
                for face_landmarks in landmarks:
                    ...
                    cv2.putText(image,"y: " + str(np.round(y,2)),(500,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
